...
export function ColumnSelector<T>({ columns, onSwitchColumn }: ColumnSelectorProps<T>) {
    const [open, setOpen] = useState<boolean>(false);
    const [optionsState, setOptionsState] = useState<Map<string, ColumnSelectorItem>>(new Map());
    ...
    const checkboxes = Array.from(optionsState).map(
        ([key, { displayName }]) =>
            ({
                key,
                label: (
                    <Checkbox value={key} disabled={(key === 'all' && optionsState.get('all')?.isSelected) || (key === 'default' && optionsState.get('default')?.isSelected)} checked={optionsState.get(key)?.isSelected}>
                        {displayName}
                    </Checkbox>
                ),
    ...
    return (
        <Dropdown open={open} trigger={['click']} menu={menuProps as unknown as MenuProps}>
